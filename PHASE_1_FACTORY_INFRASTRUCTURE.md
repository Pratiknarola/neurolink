...
export type DomainExecutionContext<T = Record<string, unknown>> = ExecutionContext<{
  domainType?: string;
  domainConfig?: DomainConfig;
  providerConfig?: {
    token?: string;
...
export interface DomainEvaluationCriteria {
  ...
  alertSeverityMapping: {
    ...
    medium: { relevanceRange: [number, number]; accuracyRange: [number, number] };
...
export class DomainConfigurationFactory {
  ...
  static createDomainConfig(options: DomainConfigOptions): DomainConfig {
    const { domainType, customConfig = {}, templateOverrides = {}, validationEnabled = true } = options;
    
    logger.debug(`Creating domain config for: ${domainType}`);

    // Get base template or create default
    const template = this.domainTemplates.get(domainType) || this.createDefaultTemplate(domainType);
    
    // Merge configurations
    const domainConfig: DomainConfig = {
      domainName: domainType,
      domainDescription: template.baseConfig.domainDescription || `Expert in ${domainType}`,
      keyTerms: template.baseConfig.keyTerms || [],
      failurePatterns: template.baseConfig.failurePatterns || this.getDefaultFailurePatterns(),
      successPatterns: template.baseConfig.successPatterns || this.getDefaultSuccessPatterns(),
      evaluationCriteria: template.baseConfig.evaluationCriteria || this.getDefaultEvaluationCriteria(),
   ...
   */
  static getDomainEvaluationCriteria(domainType: DomainType): DomainEvaluationCriteria {
    const template = this.domainTemplates.get(domainType);
    return template?.baseConfig.evaluationCriteria || this.getDefaultEvaluationCriteria();
  }
  ...
  private static validateDomainConfig(config: DomainConfig, template: DomainTemplate): void {
    ...
    if (template.validationRules) {
      for (const rule of template.validationRules) {
        if (!rule.validator(config[rule.field])) {
          throw new Error(`Domain config validation failed: ${rule.errorMessage}`);
        }
      }
    }
  }
...
DomainConfigurationFactory.registerDomainTemplate({
  ...
  baseConfig: {
    ...
    failurePatterns: ['no data available', 'insufficient metrics', 'data incomplete'],
    successPatterns: ['analysis shows', 'data indicates', 'metrics reveal', 'trend analysis'],
    evaluationCriteria: {
...
DomainConfigurationFactory.registerDomainTemplate({
  ...
  baseConfig: {
    ...
    failurePatterns: ['medical information unavailable', 'cannot provide medical advice', 'insufficient patient data'],
    successPatterns: ['clinical analysis shows', 'medical data indicates', 'patient outcomes demonstrate'],
    evaluationCriteria: {
    ...
    toolPreferences: ['medical_analyzer', 'patient_data_processor', 'clinical_evaluator']
  },
  requiredFields: ['domainName', 'domainDescription', 'keyTerms'],
  optionalFields: ['evaluationCriteria', 'toolPreferences']
});
...
export class ContextConverter {
  ...
  static convertBusinessContext(
    ...
    return {
      ...
      config: {
        domainType,
        providerConfig: {
          token: legacyContext.juspayToken || legacyContext.apiToken || legacyContext.authToken,
          endpoint: legacyContext.apiEndpoint || legacyContext.serviceUrl,
          provider: this.inferProvider(legacyContext)
        },
        platformConfig: {
          type: legacyContext.shopType || legacyContext.platformType || 'generic',
  ...
  private static extractCustomData(context: Record<string, unknown>): Record<string, unknown> {
    const knownFields = new Set([
      'sessionId', 'userId', 'juspayToken', 'shopUrl', 'shopId', 'shopType',
      'merchantId', 'enableDemoMode', 'platformIntegrations', 'apiToken',
      'apiEndpoint', 'serviceUrl', 'platformType', 'platformUrl', 'platformId',
      'environment', 'region', 'enabledFeatures'
    ]);
...
import type { DomainConfig, DomainConfigOptions } from '../types/domainTypes.js';
import type { ExecutionContext } from '../mcp/contracts/mcpContract.js';
...
export function enhanceOptionsWithDomain(
  ...
  const domainConfig = DomainConfigurationFactory.createDomainConfig(domainOptions);
...
export function enhanceOptionsWithToolDiscovery(
  ...
  logger.debug(`Enhancing options with tool discovery: ${discoveryConfig.labels.join(', ')}`);
...
export function enhanceOptionsComprehensive(
  ...
  if (enhancement.toolDiscovery) {
    enhanced = enhanceOptionsWithToolDiscovery(enhanced, enhancement.toolDiscovery);
  }
...
// Helper functions
function calculateTerminologyAccuracy(content: string, keyTerms: string[]): number {
...
function generateDomainSuggestions(content: string, domainConfig: DomainConfig): string[] {
  ...
  const missingTerms = domainConfig.keyTerms.filter(term =>
    !lowerContent.includes(term.toLowerCase())
  );
  
  if (missingTerms.length > 0) {
    suggestions.push(`Consider including key terms: ${missingTerms.slice(0, 3).join(', ')}`);
  }
...
import type { DomainTemplate, DomainConfig } from '../../src/lib/types/domainTypes.js';
...
describe('DomainConfigurationFactory', () => {
  ...
  describe('Domain Template Registration', () => {
    it('should register a custom domain template', () => {
      ...
      expect(DomainConfigurationFactory.isDomainRegistered('test-domain')).toBe(true);
      expect(DomainConfigurationFactory.getAvailableDomains()).toContain('test-domain');
    });
  ...
  describe('Domain Configuration Creation', () => {
    ...
    it('should merge custom configuration with template', () => {
      ...
      expect(config.keyTerms).toEqual(['medical', 'patient', 'clinical', 'treatment']);
  ...
  describe('Evaluation Criteria', () => {
    it('should return domain-specific evaluation criteria', () => {
      ...
      const criteria = DomainConfigurationFactory.getDomainEvaluationCriteria('finance');
    ...
    it('should return default criteria for unknown domain', () => {
      const criteria = DomainConfigurationFactory.getDomainEvaluationCriteria('unknown-domain');
...
import { enhanceOptionsWithDomain, enhanceOptionsComprehensive, createExecutionContextFromOptions } from '../../src/lib/utils/optionsUtils.js';
...
describe('Factory Integration', () => {
  describe('Options Enhancement Integration', () => {
    it('should integrate domain enhancement with existing interfaces', () => {
      ...
      const enhanced = enhanceOptionsWithDomain(
        baseOptions,
        { domainType: 'analytics' }
      );
    ...
    it('should support comprehensive enhancement', () => {
      ...
      const enhanced = enhanceOptionsComprehensive(
        baseOptions,
        {
  ...
  describe('Context Conversion Integration', () => {
    it('should convert legacy business context to generic domain context', () => {
      ...
      expect(converted.config?.platformConfig?.url).toBe('https://shop.example.com');
  ...
  describe('ExecutionContext Creation', () => {
    it('should create ExecutionContext from enhanced options', () => {
      ...
      const execContext = createExecutionContextFromOptions(
        enhancedOptions,
        { sessionId: 'test_session', userId: 'test_user' }
      );
      ...
      expect(execContext.metadata?.enhancementType).toBe('domain-configuration');
    });
  });
});
```
...
**Task Group 1.1.1: Domain Types Interface** ⏱️ *Est: 3-4 hours*
...
**Task Group 1.1.2: Domain Configuration Factory** ⏱️ *Est: 6-8 hours*
...
**Task Group 1.1.3: Core Types Extension** ⏱️ *Est: 2-3 hours*
...
**Task Group 1.2.1: MCP Contract Extension** ⏱️ *Est: 2-3 hours*
...
**Task Group 1.2.2: Context Conversion Utilities** ⏱️ *Est: 4-5 hours*
...
**Task Group 1.3.1: Generate Types Extension** ⏱️ *Est: 3-4 hours*
...
**Task Group 1.3.2: Options Enhancement Utilities** ⏱️ *Est: 6-8 hours*
...
**Task Group 1.4.1: Domain Configuration Tests** ⏱️ *Est: 4-5 hours*
...
**Task Group 1.4.2: Integration Tests** ⏱️ *Est: 4-5 hours*
...
**Task Group 1.4.3: Extend Existing Streaming Tests** ⏱️ *Est: 3-4 hours*
...
**Task Group 1.4.4: CLI Verification & Impact Assessment** ⏱️ *Est: 5-6 hours*
...
**Task Group 1.4.5: Evaluation System Integration** ⏱️ *Est: 3-4 hours*
...
**Task Group 1.4.6: Documentation Enhancement** ⏱️ *Est: 4-5 hours*
...
- [ ] **Feature Branch**: `git checkout -b phase-1-factory-infrastructure` *(completed at start)*
- [ ] **Incremental Commits**: ✅ *8 checkpoint commits completed throughout development*
- [ ] **Final Phase Commit**: ✅ *Complete Phase 1 implementation*  
